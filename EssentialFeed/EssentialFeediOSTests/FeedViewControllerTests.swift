...
final class FeedViewControllerTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expcted another loading request once user initiates a load")
        
        sut.simulateUserInitiatedFeedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expcted a third loading request once user initiates another load")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator indicator state change for first view once second image loading completes with error")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected no retry action change state for first view once second image loading completes with error")
    ...
    func test_feedImageViewRetryButton_retriesImageLoad() {
        ...
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected only image URL requests before retry action")
    ...
    private func assertThat(_ sut: FeedViewController, hasViewConfiguredFor image: FeedImage, at index: Int, file: StaticString = #file, line: UInt = #line) {
        ...
        guard let cell = view as? FeedImageCell else {
            return XCTFail("Expected \(FeedImageCell.self) intance, got \(String(describing: view)) instead", file: file, line: line)
        }
